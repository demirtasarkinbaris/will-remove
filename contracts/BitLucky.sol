...
contract BitLucky is ERC1155, Ownable {
    ...
    function buyTickets(uint256 productID, uint256 ticketAmount) external {
        ...
        if (products[productID].ticketsSold == products[productID].maxTickets) {
            products[productID].isAllSold = true;
        }
    ...
    // Fallback function to revert any Ether sent to this contract
    receive() external payable {
        revert(
            "This contract does not accept Ether directly. Use the buyTicket function."
        );
    }
}
